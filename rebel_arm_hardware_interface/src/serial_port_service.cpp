...
#include "rebel_arm_hardware_interface/serial_port_service.hpp"

using namespace rebel_arm_hardware_interface;
...
void SerialPortService::read()
{
    ...
    for (unsigned int i = 0; i < bytes_transferred; ++i) {
        ...
        {
            p = (char*) &rebelArmFeedback;
            *p++ = prev_byte;
            *p++ = read_buf_raw[i];
            msg_counter = 2;
        }
        else if (msg_counter >= 2 && msg_counter < sizeof(RebelArmFeedback)) {
            *p++ = read_buf_raw[i];
            msg_counter++;
        }

        if (msg_counter == sizeof(RebelArmFeedback))
        {
            rebelArmFeedbackCallback(rebelArmFeedback);
...
void SerialPortService::onReceive(const boost::system::error_code&, size_t)
{
...
void SerialPortService::BindRebelArmFeedbackCallback(std::function<void(RebelArmFeedback)> fn) {
    rebelArmFeedbackCallback = fn;
}